    ...
    for bank in range(NUM_BANKS):
      if mem_dtm != 'off':
        if (bank_mode_trace[bank] == NORMAL_POWER):
            accesses_read[bank] =  accesses_read[bank] + accesses_read_lowpower[bank]
            accesses_write[bank] =  accesses_write[bank] + accesses_write_lowpower[bank]
            accesses_read_lowpower[bank] = 0
            accesses_write_lowpower[bank] = 0
        if (bank_mode_trace[bank] == LOW_POWER):
            accesses_read_lowpower[bank] =  accesses_read[bank] + accesses_read_lowpower[bank]
            accesses_write_lowpower[bank] =  accesses_write[bank] + accesses_write_lowpower[bank]
        ...
        ## print(bank)
        ## print('the normal bank  write accesses are  ')
        ## print(accesses_write[bank])
        ## print('the normal bank  read accesses are  ')
        ## print(accesses_read[bank])
        ## print('the lowpower bank  write accesses are  ')
        ## print(accesses_write_lowpower[bank])
        ## print('the lowpower bank  read accesses are  ')
        ## print(accesses_read_lowpower[bank])
